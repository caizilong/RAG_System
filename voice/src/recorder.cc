...
Recorder::Recorder(ConversationManager& conv_manager) : conv_(conv_manager), stop_(false) {
    ...
    PaError err = Pa_OpenStream(&stream_, &param, nullptr, mic_sample_rate_, 0, paClipOff,
                                RecordCallback, &conv_);
...
int32_t Recorder::RecordCallback(const void* input_buffer, void*, unsigned long frames,
